...
export default function AdminPage() {
    ...
    const handleDeleteMember = async (subCommitteeId, memberId) => {
        ...
        try {
            const res = await fetch(`/api/sub-committee?subCommitteeId=${subCommitteeId}&memberId=${memberId}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
    ...
    const handleDeleteCoreMember = async (group, memberId) => {
        ...
        try {
            const res = await fetch(`/api/core-committee?group=${group}&id=${memberId}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
